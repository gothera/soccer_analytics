...
from datatools.ellipse_utils import add_conic_points

import matplotlib.pyplot as plt
import matplotlib.image as mpimg
from typing import Dict, Tuple, Union

def plot_image_with_keypoints(image_path: str, keypoints: Dict[int, Union[Tuple[float, float], None]]) -> None:
    # Load the image
    ...
    # Iterate over the dictionary and plot the keypoints
    for key, point in keypoints.items():
        if point is not None:
            x, y = point
            print(key, point, "Da")
            plt.scatter(x, y, c='red', s=40, marker='x')  # Mark the point on the image
            # plt.text(x + 5, y + 5, str(key), color='white', fontsize=12, bbox=dict(facecolor='black', alpha=0.5))
...
if __name__ == "__main__":
    ...
    sample = read_annot('./valid/00470.json')
    keypoints = get_intersections(sample)[0]
    plot_image_with_keypoints('./valid/00470.jpg', keypoints)
